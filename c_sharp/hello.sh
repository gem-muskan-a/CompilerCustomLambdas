function handler () {
    ...
    head -n -8 /tmp/Sample.txt > /tmp/Sample.cs
    echo $(cat /tmp/Sample.cs)
    ...
    if csc /tmp/Sample.cs > /tmp/CompilationError.txt; then
        echo "Compilation successful"
        i=1
        json="["
        while [ $i -le $noOfTestCases ]
        do
            time -f "Time(s): %e Memory(Kb): %M " timeout 2 /tmp/Sample.exe < /mnt/efs/$qid/input$i.txt > /tmp/useroutput$i 2> /tmp/error$i
